...
type ProductUpdateModalProps = {
    ...
    setOpenUpdateProductModal: React.Dispatch<React.SetStateAction<boolean>>,
}
    ...
    const handelSubmit = async () => {
        try {
            ...
            if(Object.keys(newErrors).length === 0) {
                ...
                const response = await updateProduct(token, selectedProduct.id, formObj);
                if (response.status === 200) {
                    // success popup for update product success
                    Swal.fire({
                        title: "Success..!",
                    ...
                    resetForm();
                    setOpenUpdateProductModal(!openUpdateProductModal);
                    // error popup for error while update product
                    Swal.fire({
                        title: "Ooops..!",
            ...
            <div className="relative max-w-2xl w-full border rounded-lg">
                <div className="flex flex-col p-5 rounded-lg shadow bg-white">
                    ...
                    <div className="flex gap-2 w-full py-3">
                        ...
                        />
                        <div className="relative mb-3 w-full">
                            <label id="image" className="leading-7 text-sm text-gray-600">Image</label>
                            <input 
                                type="file" 
                                id="image" 
                                accept=".jpg,.jpeg,.png"
                    ...
                    <div className="flex justify-end items-center mt-3">
                        <button 
                            ...
                            onClick={() => setOpenUpdateProductModal(!openUpdateProductModal)}
                        >
                            Cancel
                        </button>
                        <button 
